...
def associate(a, mags_a, b, mags_b, dr, dmag, confs_b = None, sigfs_b = None, frac_flux_thresh = None):
    ...
    goodmatch = np.zeros(mags_a.size, bool)
...
def clusterize_spire(seed_cat, cat_x, cat_y, cat_n, cat_fs, max_num_sources, nsamp, search_radius, band_names=None):
    ...
    j = 0
    for i in range(cat_n.size): # don't have to for loop to stack but oh well
        n = cat_n[i]
...
def generate_seed_catalog(kd, cat_all, cat_f0_all, PCi, matching_dist, verbose=False):
    ...
    G = nx.Graph()
    G.add_nodes_from(range(0, cat_all.shape[0]))

    for i in range(cat_all.shape[0]):
        for j in matches[i]:
    ...
    current_catalogue = 0
    #print('HELLOOO', np.shape(PCi), np.shape(matches))
    for i in range(cat_all.shape[0]):
        matches[i].sort()
        bincount = np.bincount(PCi[matches[i]]).astype(int)
        ending = np.cumsum(bincount).astype(int)
        starting = np.zeros(bincount.size).astype(int)
        ...
        for j in range(bincount.size):
            ...
            if bincount[j] > 1:
                ...
                l = -1
                for k in range(starting[j], ending[j]):
                    m = matches[i][k]
...
class cross_match_roc():
    ...
    def load_chain(self, path):
        ...
        self.lion_cat = dict({'n':lion['n'][-self.nsamp:].astype(int), 'x':lion['x'][-self.nsamp:,:], 'y':lion['y'][-self.nsamp:,:], \
    ...
    def compute_kd_tree(self, chain_path=None, mode='lion', mask_hwhm=0., apply_noise_mask=False):
        ...
        if self.image is not None and apply_noise_mask:
            noise_mask = (self.image[cat['y'].astype(int), cat['x'].astype(int)] != 0.0)
            noise_mask *= (self.error[cat['y'].astype(int), cat['x'].astype(int)] < self.max_noise_level)
            mask *= noise_mask
        ...
        print(np.shape(mask), np.shape(PCi))
        PCi = PCi[mask[0:self.nsamp]].flatten() #change made here!!!!!!
