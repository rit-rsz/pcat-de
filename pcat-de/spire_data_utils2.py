...
import os
import sys
sys.path.append("/home/djk3679/szeffect/pcat/pcat-de/pcat-de")
#from pcat_main import *
import pcat_main as pcat
import params
...
def get_empirical_psf_template(ind):
	empirical_beams = ["/home/djk3679/szeffect/data/SPIRE/photometer_empirical_beam/resampled_beam_PSW.fits".replace('PSW', j) for j in ['PSW', "PMW", "PLW"]]
	beam = fits.open(empirical_beams[ind])['image'].data
	nbin = 5 # should really calculate nbin more dynamically based on actual pixelization of maps, but 6 suffices for the higher resolution maps
	nc = nbin**2
	cf = psf_poly_fit(beam, nbin)

	return beam, cf, nc, nbin
...
def load_param_dict(timestr=None, result_path=params.result_basedir, encoding=None):
...
class pcat_data():
	...
	def load_in_data(self, gdat, map_object=None, tail_name=None, show_input_maps=False, \
		...
		if flux_density_conversion_dict is None:
			...
			flux_density_conversion_dict = gdat.flux_density_conversion_dict
		...
		for i, band in enumerate(gdat.bands):
					...
					for t, template_name in enumerate(gdat.template_names):
						...
						if gdat.band_dict[band] in self.template_bands[template_name]:
							...
							if gdat.template_filename is not None and template_name=='sze':
								...
								print('template file name is', file_name + f'[{template_name}]')
							...
							if gdat.inject_sz_frac is not None and template_name=='sze':
								...
								# template_inject = gdat.inject_sz_frac*template*flux_density_conversion_dict[gdat.band_dict[band]]
								# print('injected amp is '+str(np.round(gdat.inject_sz_frac*temp_mock_amps_dict[gdat.band_dict[band]]*flux_density_conversion_dict[gdat.band_dict[band]], 4)))
								print('injected amp is '+str(np.round(gdat.inject_sz_frac*temp_mock_amps_dict[gdat.band_dict[band]], 4))) #*flux_density_conversion_dict[gdat.band_dict[band]]
								print('Template inject max:', np.max(template_inject))
								...
								if show_input_maps:
									...
									plt.subplot(1,2,1)
									plt.title('injected amp is '+str(np.round(gdat.inject_sz_frac*temp_mock_amps_dict[gdat.band_dict[band]], 4))) # *flux_density_conversion_dict[gdat.band_dict[band]]
			...
			if gdat.auto_resize:
				...
				for t, template in enumerate(template_list):
					...
					if template is not None:
						...
						if gdat.template_order[t] == 'dust' or gdat.template_order[t] == 'planck':
							...
							print("BAD BAD BAD subtraction for sz being made") # - ryan
				...
				variance[variance==np.nan]=np.inf # addition to deal with images that have nans - ryan
				weight = 1. / variance
			...
			# added empirical_beam
			if gdat.use_empirical_beam:
				print("USING EMPIRICAL BEAM")
				psf, cf, nc, nbin = get_empirical_psf_template(i)
